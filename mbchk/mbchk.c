...
//it was 8192 before i hacked it
#define SEARCH 8192
static int
check_multiboot (const char *filename, FILE *fp)
{
  ...
  if (fread (buf, 1, SEARCH, fp) < 0)
	...
	printf("Looking for magic number 0x%x\n", MULTIBOOT_HEADER_MAGIC);
  for (i = 0; i < SEARCH - sizeof (multiboot_header_t); i++)
    {
